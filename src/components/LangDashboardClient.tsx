...
export function LangDashboardClient({
  ...
  useEffect(() => {
    if (user) {
      getProgress(user.id, langKey).then(({ data }) => {
        if (data) {
          const slugs = new Set(
            (data as ProgressItem[]).filter(p => p.completed).map(p => p.lesson_slug)
          );
  ...
  const escales = lessons.reduce(
    (acc, lesson) => {
      ...
      if (!acc[key]) acc[key] = { title: lesson.escaleTitle, lessons: [] };
  ...
  return (
    <div className="page-enter pt-24 pb-20 px-4 sm:px-6">
      {/* ===== LAYOUT 3 COLONNES (desktop) / 1 colonne (mobile) ===== */}
      <div className="max-w-[1400px] mx-auto flex gap-6">

        {/* ===== COLONNE GAUCHE — Alphabet (sticky, desktop only) ===== */}
        <div className="hidden lg:block w-[220px] xl:w-[260px] flex-shrink-0">
          <div className="sticky top-24">
            <AlphabetSidebar langKey={langKey} color={lang.color} />
          </div>
        </div>
        ...
        <div className="flex-1 min-w-0 max-w-3xl">
          ...
          <div className="mb-8">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h1 className="font-display text-3xl sm:text-4xl" style={{ color: lang.color }}>
                  {lang.name}
                </h1>
          ...
          <div className="lg:hidden mb-6">
            <AlphabetSidebar langKey={langKey} color={lang.color} collapsible />
          </div>
          ...
          {user && !loading && completedCount > 0 && (
            <div className="bg-white rounded-xl p-5 border border-black/5 mb-6">
              <div className="flex items-center justify-between mb-2">
                ...
                <p className="text-sm text-gray-400">{completedCount}/{totalLessons} leçons · {progressPercent}%</p>
              </div>
              <div className="h-2.5 bg-gray-100 rounded-full overflow-hidden">
                <div
                  className="h-full rounded-full transition-all duration-500"
                  style={{ width: `${progressPercent}%`, background: lang.color }}
                />
              </div>
            </div>
          )}
          ...
          {nextLesson && (
            <Link
              ...
              className="flex items-center justify-between bg-white rounded-xl p-5 border-2 no-underline mb-8 transition-all hover:-translate-y-0.5 hover:shadow-lg group"
              style={{ borderColor: `${lang.color}30` }}
              onMouseEnter={(e) => { (e.currentTarget as HTMLElement).style.borderColor = lang.color; }}
              onMouseLeave={(e) => { (e.currentTarget as HTMLElement).style.borderColor = `${lang.color}30`; }}
            >
              ...
              <span className="text-gray-300 text-xl group-hover:text-gray-500 transition-colors">→</span>
            </Link>
          )}
          ...
          {escaleEntries.length > 0 ? (
            <div className="flex flex-col gap-3">
              {escaleEntries.map(([escaleNum, escale]) => {
                ...
                const allDone = escaleCompleted === escaleTotal && escaleCompleted > 0;

                return (
                  <div
                    key={escaleNum}
                    className={`bg-white rounded-xl border overflow-hidden transition-colors ${isOpen ? 'border-black/10' : 'border-black/5'}`}
                  >
                    <button
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <p className="text-xs font-bold tracking-wider text-gray-400 uppercase">Escale {escaleNum}</p>
                          {user && !loading && (
                            <span className="text-xs text-gray-300">{escaleCompleted}/{escaleTotal}</span>
                          )}
                        </div>
                        <p className="font-display text-base sm:text-lg text-ink truncate">{escale.title}</p>
                        {user && !loading && escaleCompleted > 0 && (
                          <div className="mt-2 h-1 bg-gray-100 rounded-full overflow-hidden max-w-[200px]">
                            <div className="h-full rounded-full" style={{ width: `${escalePercent}%`, background: lang.color }} />
                          </div>
                        )}
                      </div>
                            ...
                            return (
                              <Link
                                ...
                                className={`flex items-center gap-3 p-3 rounded-lg no-underline transition-all hover:-translate-y-0.5 ${
                                  isDone ? 'bg-emerald-50/50 hover:bg-emerald-50' : 'bg-gray-50/50 hover:bg-gray-50'
                                }`}
                              >
                                <div
                                ...
                                <div className="flex-1 min-w-0">
                                  <p className={`text-sm font-medium truncate ${isDone ? 'text-emerald-700' : 'text-ink'}`}>
                                    {lesson.title}
                                  </p>
                                  {lesson.description && (
            ...
            <div className="bg-white rounded-2xl p-10 border border-black/5 text-center">
              ...
              <h3 className="font-display text-xl mb-2 text-ink">Premières leçons en préparation</h3>
              <p className="text-sm text-gray-500 max-w-md mx-auto leading-relaxed">
                On travaille sur les premières escales en {lang.name.toLowerCase()}.
              </p>
              <Link href="/#langues" className="inline-block mt-6 bg-ink text-white text-sm font-semibold px-6 py-3 rounded-xl no-underline hover:opacity-90 transition-opacity">
